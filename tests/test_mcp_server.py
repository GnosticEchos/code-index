...
class TestCodeIndexMCPServer:
    ...
    async def test_validate_services_success(self, temp_config_file, mock_resource_manager):
        ...
        with patch('src.code_index.embedder.OllamaEmbedder') as mock_embedder_class:
            ...
            # Mock successful Qdrant validation
    ...
    async def test_validate_services_qdrant_failure(self, temp_config_file):
        ...
        with patch('src.code_index.embedder.OllamaEmbedder') as mock_embedder_class:
            ...
            # Mock Qdrant validation failure
            with patch('src.code_index.vector_store.QdrantVectorStore') as mock_qdrant_class:
    ...
    def test_register_tools(self, temp_config_file, mock_fastmcp):
        """Test tool registration."""
        server = CodeIndexMCPServer(temp_config_file)
        server._mcp = mock_fastmcp
        
        # Mock the tool modules
        with patch('src.code_index.mcp_server.tools.index_tool.index') as mock_index:
            with patch('src.code_index.mcp_server.tools.index_tool.create_index_tool_description') as mock_index_desc:
                with patch('src.code_index.mcp_server.tools.search_tool.search') as mock_search:
                    with patch('src.code_index.mcp_server.tools.search_tool.create_search_tool_description') as mock_search_desc:
                        with patch('src.code_index.mcp_server.tools.collections_tool.collections') as mock_collections:
                            with patch('src.code_index.mcp_server.tools.collections_tool.create_collections_tool_description') as mock_collections_desc:
                                
                                mock_index_desc.return_value = "Index tool description"
                                mock_search_desc.return_value = "Search tool description"
                                mock_collections_desc.return_value = "Collections tool description"
                                ...
                                calls = mock_fastmcp.tool.call_args_list
                                
                                # Index tool
                                assert calls[0][1]['name'] == 'index'
                                assert calls[0][1]['description'] == "Index tool description"
                                
                                # Search tool
                                assert calls[1][1]['name'] == 'search'
                                assert calls[1][1]['description'] == "Search tool description"
                                
                                # Collections tool
                                assert calls[2][1]['name'] == 'collections'
                                assert calls[2][1]['description'] == "Collections tool description"
