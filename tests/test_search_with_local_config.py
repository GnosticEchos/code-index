#!/usr/bin/env python3
"""
Direct test of search functionality with proper embedding dimensions.
"""
import os
import sys
...
def test_search():
    ...
    # Load configuration
    config_path = os.path.join(os.path.dirname(__file__), 'local_code_index.json')
    if os.path.exists(config_path):
        config = Config.from_file(config_path)
    else:
        config = Config()
        config.workspace_path = os.path.dirname(os.path.abspath(__file__))
